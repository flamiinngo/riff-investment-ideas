...
export async function requestBasketQuote(
  idea: Idea,
  amount: number,
  taker: Address,
) {
  ...
  if (!response.ok)
    throw new Error(result.error ?? 'A live basket quote is unavailable.');
  return result;
}
...
export async function sendAtomicBasket(quote: BasketQuote, account: Address) {
  ...
  if (allowanceTarget) {
    const total = quote.quotes.reduce(
      (sum, item) => sum + BigInt(item.sellAmount),
      0n,
    );
    calls.push({
      to: quote.sellToken,
  ...
  for (const item of quote.quotes)
    calls.push({
      ...
      value: item.transaction.value
        ? `0x${BigInt(item.transaction.value).toString(16)}`
        : '0x0',
    });
  let callsId: string;
  try {
    callsId = (await ethereum.request({
    ...
    if (/unsupported|method/i.test(message))
      throw new Error(
        'This wallet cannot execute the basket atomically. Open Riff with Base Account.',
      );
    throw error;
  }
  for (let attempt = 0; attempt < 80; attempt += 1) {
    ...
    const status = (await ethereum.request({
      ...
      params: [callsId],
    })) as { status?: number; receipts?: { transactionHash?: Hex }[] };
    if (status.status === 200)
      return status.receipts
        ?.map((receipt) => receipt.transactionHash)
        .filter(Boolean) as Hex[];
    if (status.status === 400)
      throw new Error('The basket reverted. No partial purchase was accepted.');
  }
  throw new Error(
    'The wallet is still confirming. Check BaseScan before trying again.',
  );
}
